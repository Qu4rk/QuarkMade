import { assetPath } from "../lib/site";

/** Structured content for QuarkMade digital craft studio website. */

export const tileData = [
  {
    content: "AVAILABILITY",
    styles: {
      fontWeight: 500,
      textColor: "var(--color-gold)",
    },
  },
  {
    content: "OPEN FOR COMMISSIONS",
    styles: {
      fontWeight: 500,
      textColor: "var(--foreground)",
    },
  },
  {
    content: "STUDIO 2026",
    styles: {
      fontWeight: 500,
      textColor: "var(--foreground)",
    },
  },
];

export const cards = [
  {
    variant: "quietide",
    title: "QuieTide — Mediterranean Digital Sanctuary",
    summary:
      "A curated digital experience capturing Mediterranean light and rhythmic coastal stillness through WebGL shaders, smooth motion, and contemplative pacing.",
    tags: ["MEDITERRANEAN", "WEBGL", "WELLNESS"],
    image: assetPath("/assets/portfolio/quietide_capture_space.webp"),
    link: "https://qu4rk.github.io/quietide-website/",
  },
  {
    variant: "lumina-living",
    title: "Lumina Living — Architectural Spaces & Modern Living",
    summary:
      "An architectural real estate and living sanctuary platform crafted with panoramic media viewers, bespoke typography, and seamless property discovery.",
    tags: ["ARCHITECTURE", "RESIDENTIAL", "EDITORIAL"],
    image: assetPath("/assets/portfolio/lumina_signature_living.webp"),
    link: "http://www.lumina-living.net/",
  },
  {
    variant: "chronotomi",
    title: "Chronotomi — Haute Horlogerie & Timepiece Showcase",
    summary:
      "A bespoke luxury horology flagship featuring precision kinetic typography, interactive timepiece showcases, and editorial storytelling for collectors.",
    tags: ["HOROLOGY", "LUXURY", "INTERACTIVE"],
    image: assetPath("/assets/portfolio/chronotomi_collection.webp"),
    link: "http://www.chronotomi.com/",
  },
  {
    variant: "craft-philosophy",
    title: "The Anatomy of High-Craft Web Design",
    summary:
      "How purposeful motion, typographic rigor, and bespoke interaction design transform digital products into memorable brand flagships.",
    tags: ["STUDIO ESSAY", "MOTION DESIGN"],
    image: assetPath("/assets/portfolio/chronotomi_standard.webp"),
    link: "#inquire",
  },
];

export const capabilities = [
  {
    title: "Bespoke Web Design",
    description:
      "Custom visual identities, editorial layouts, and design systems tailored for ambitious brands.",
  },
  {
    title: "Creative Engineering",
    description:
      "High-performance Next.js architectures, fluid micro-animations, and interactive WebGL experiences.",
  },
  {
    title: "Digital Art Direction",
    description:
      "End-to-end creative direction, sound design integration, and visual storytelling that commands attention.",
  },
];

export const logos = [
  {
    name: "QuieTide",
    label: "QUIETIDE",
    logo: assetPath("/assets/portfolio/logos/quietide-logo.webp"),
    href: "https://qu4rk.github.io/quietide-website/",
  },
  {
    name: "Lumina Living",
    label: "LUMINA LIVING",
    logo: assetPath("/assets/portfolio/logos/lumina-logo.webp"),
    href: "http://www.lumina-living.net/",
  },
  {
    name: "Chronotomi",
    label: "CHRONOTOMI",
    logo: assetPath("/assets/portfolio/logos/chronotomi-logo.webp"),
    href: "http://www.chronotomi.com/",
  },
  {
    name: "WebGL",
    label: "WEBGL & THREE.JS",
  },
  {
    name: "Next.js",
    label: "NEXT.JS",
  },
];

export const textLinkData = [
  { text: "Privacy Policy", href: "/privacy" },
  { text: "Terms of Service", href: "/terms" },
  { text: "Contact", href: "mailto:liasides.elias@gmail.com" },
];


